#!/bin/bash
FILES="../FSM_Generation/results/FSMs/Arduino/50K/Buses_light/*"
TESTS="../FSM_Generation/results/FSMs/Arduino/50K/Tests/*"
for f in $FILES
do
echo $(basename "$f")
for i in "$f"/*
...
sigrok-cli -d fx2lafw --config samplerate=100000 --time 10000 --channels D2 > "./Evaluation/50K_light/"$(basename "$f")"/Bus_Load/"$(basename "$j")".txt"
echo "Done sampling $i"
echo "Write external time measurement to file..."
esptool --chip auto --port /dev/ttyUSB0 run
timeout 15s tail -f < /dev/ttyUSB0 > "./Evaluation/50K_light/"$(basename "$f")"/CPU_Util/"$(basename "$j")".txt"
